...
def test_new_wording_says_collected_but_timed_out():
    ...
    # §RPT-7 货 3 ①：小红书那一章 `cited=12`，角标真进了正文——对它写「正文未能引用
    # 它们」是假话，所以这一行走的是不认领死因的那句。Reddit（cited=0）措辞一字未改。
    assert lines[0] == ("| 小红书（豆包） | 采到 296 条，已入库并参与评级与统计；"
                        "这一段没能记成完整的一段，但内容本身没丢 |")
