...
- 09-16 调度复验通过并批准范围变更：**加做货 4**（原挂账 1 升格），待早上向用户追认。
...
```
../Owli/.venv/bin/python -m pytest -q > /tmp/d072_pytest_g4.txt 2>&1; echo exit=$?
exit=0
2169 passed, 3 skipped in 31.54s
```

基线 2161 passed / 3 skipped → 三货后 2165 / 3 → **货 4 后 2169 / 3**，
+8 正是本包新增的八条用例（货 2 一条、货 3 三条、货 4 四条）。
货 4 判据 ③「不低于 2165/3」满足。
...
## 货 4 · 主张行与态度行同源（`app/report/polish/run.py`）

> 调度 09-16 批准的**范围变更**（原挂账第 1 条升格为货 4，待早上向用户追认）：
> 它与货 3 是同一条失灵路，本轮不显形只因该稿 `confidence_line()` 为空；
> 不修则换份有交叉验证读数的稿就复发（老教训「一个缺陷两个成因：修完第一个
> 要走一遍相反的那一支」）。

`_inject_after_confidence()`（管「主张 N 条：单源 7 / 多源互证 2。」那行）原先同样
只认引用块里的把握度 `startswith(">")`——把握度写成普通段落时匹配失灵，主张行
被兜底扔到节末。

**判据 ① 同源**：两个注入点现在共用同一套锚定与兜底，没有另造一套——

| 共用件 | 作用 | 立于 |
|---|---|---|
| `_confidence_index()` | 锚把握度：认词不认 `>` | 货 3 |
| `_block_end(lines, start, stops=…)` | 跨段：一段 = **连续非空行**，引用块与普通段落同形；`stops` 给发现列表那处加提前刹车 | 货 4 抽出 |
| `_append_at_section_end()` | 末级兜底：锚点全找不着才接节末（老行为） | 货 4 抽出 |

原先两处各写一份跨段规则（一处认 `>` 前缀、一处认非空行），换个写法就会一处
跟得上一处跟不上——这正是本缺陷两次现形的同一个根。

### 判据 ② 红绿两侧读数（`-k D072_货4`）

| 用例 | 老代码 | 新代码 |
|---|---|---|
| 把握度是普通段落 → 主张行紧跟其后 | **FAIL**「主张行没紧跟把握度段」（落到了节末） | PASS |
| 把握度是**多行**普通段落 → 不插进段落中间 | **FAIL**「主张行插进把握度段中间了」 | PASS |
| 把握度是引用块 → 落点一格不动（回归锁） | PASS | PASS |
| 把握度缺席 → 接节末（回归锁） | PASS | PASS |

合计老代码 `2 failed, 2 passed`、新代码 `4 passed`。后两条是**回归锁**，两侧皆绿是
预期：换锚定是为了多认一种形态，不是换掉原来认得的那种。

> ⚠️ 夹具踩过一次假绿：多行那条最初把把握度段写成节里**最后一段**，于是
> 「插在段后」与老代码「兜底接节末」落在同一位置，红不出来（`1 failed` 而非 2）。
> 夹具后面补一段「后文另起一段」才分得出真假，断言里写了告示。
> 又一次印证「造红要真红过一次」——不验尺子就会把没修的当修好的。

commit `ad4b68b`。

## 挂账 / 呈调度的观察（本包**未动**）

1. 货 1 / 货 3 都是**下一次出稿才生效**的改动。现有那份 09-15 正式稿里的病象 1、3
